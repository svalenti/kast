...
def lacos(_input0, output='clean.fits', outmask='mask.fits', gain=1.3, readn=9, xorder=9, yorder=9, sigclip=4.5, sigfrac=0.5, objlim=1, verbose=True, interactive=False):
    ...
    # globals()
    import kast
    from kast.kastutil import delete
    import sys
    import re
    import os
    import string
    ...
    oldoutput, galaxy, skymod, med5 = '_oldoutput.fits', '_galaxy.fits', '_skymod.fits', '_med5.fits'
    blk, lapla, med3, med7, sub5, sigima, finalsel = '_blk.fits', '_lapla.fits', '_med3.fits', '_med7.fits', '_sub5.fits', '_sigima.fits', '_finalsel.fits'
    deriv2, noise, sigmap, firstsel, starreject = '_deriv2.fits', '_noise.fits', '_sigmap.fits', '_firstsel.fits', '_starreject.fits'
    inputmask = '_inputmask.fits'
    ...
    iraf.imcopy(_input0 + '[*,*]', '_xxx.fits', verbose='no')
    _input = '_xxx.fits'

    arrayinput, headerinput = fromfits(_input, verbose=False)
    tofits(outmask, np.float32(
    ...
    arrayoldoutput, headeroldoutput = fromfits(
...
#    m5 = ndimage.filters.median_filter(_inputarray, size=5, mode='mirror')
    ...
    arrayfirst, headerfirst = fromfits(firstsel, verbose=False)
    arraygfirst = my_convolve_with_FFT2(arrayfirst, gkernel)
    ...
    arrayfinal = my_convolve_with_FFT2(arraygfirst, gkernel)
    ...
    if npix == 0:
        stop = yes
      # delete temp files
    iraf.imcopy('_yyy.fits', output + '[*,*]', verbose='no')
...
def clean_image(img, cleanimg):
    ...
    import kast
    from kast.kastutil import delete
    array, header = fromfits(img, verbose=False)
    import warnings
    ...
    _ron = 1 
    try:
        c = cosmicsimage(array, gain=_gain, readnoise=_ron, sigclip=5.0, sigfrac=0.3, objlim=5.0, verbose=False)
    ...
    if not cleanimg:
        delete(img)
        cleanimg = img
    tofits(cleanimg, c.cleanarray, header, verbose=False)
    return cleanimg
...
def lacos_im(_input, _output='clean.fits', outmask='mask.fits', gain=1.3, readn=9, xorder=9, yorder=9, sigclip=4.5, sigfrac=0.5, objlim=1, skyval=0, niter=2, verbose=True, interactive=False):
    ...
    while stop == 'no':
        ...
        iraf.median(sigmap, med5, 5, 5, zlo='INDEF', zhi='INDEF', verbose='no')
        arraysigmap, headersigmap = fromfits(sigmap, verbose=False)
        arraymed5, headermed5 = fromfits(med5, verbose=False)
